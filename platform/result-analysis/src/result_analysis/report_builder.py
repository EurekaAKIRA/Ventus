...
def _build_task_summary_text(task_name: str, quality_status: str, summary: dict[str, Any]) -> str:
    return (
        f"任务 {task_name} 当前状态为 {quality_status}，共分析 {summary['scenario_count']} 个场景、"
        f"{summary['total_steps']} 个步骤，成功率 {summary['success_rate']}%，"
        f"平均耗时 {summary['avg_elapsed_ms']}ms，最大耗时 {summary['max_elapsed_ms']}ms。"
    )
