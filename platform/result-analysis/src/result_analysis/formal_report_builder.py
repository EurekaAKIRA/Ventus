"""Formal analysis report builder for platform task assets."""
...
def build_analysis_report(
    ...
    """Build a report that can be rendered directly by platform dashboards."""
    ...
    summary = {
        ...
        "total_requests": step_stats["performance_stats"]["total_requests"],
    ...
    report = AnalysisReport(
        ...
        execution_overview={
            ...
            "has_failures": bool(step_stats["failed_step_count"] or validation_errors),
...
def _collect_step_stats(
    ...
    failed_step_count = (
        max(total_steps - passed_step_count, len(failed_steps), metrics_failed_steps)
        if has_execution
        else 0
    )
    ...
    return {
        ...
        "success_rate": round((passed_step_count / executed_step_count) * 100, 2)
        if executed_step_count
        else 0.0,
        "failure_rate": round((failed_step_count / executed_step_count) * 100, 2)
        if executed_step_count
        else 0.0,
...
def _merge_failure_reasons(
    execution_failure_reasons: list[dict[str, Any]],
    validation_report: ValidationReport,
) -> list[dict[str, Any]]:
    merged_by_category: dict[str, dict[str, Any]] = {}
    for item in execution_failure_reasons:
        category = item["category"]
        merged_by_category[category] = {
            "category": category,
            "label": item.get("label") or _failure_category_label(category),
            "count": int(item.get("count", 0) or 0),
            "examples": list(item.get("examples") or []),
        }

    if validation_report.errors:
        merged_by_category["validation_error"] = {
            "category": "validation_error",
            "label": "Validation Error",
            "count": len(validation_report.errors),
            "examples": [{"message": message} for message in validation_report.errors[:3]],
        }
    if validation_report.warnings:
        merged_by_category["validation_warning"] = {
            "category": "validation_warning",
            "label": "Validation Warning",
            "count": len(validation_report.warnings),
            "examples": [{"message": message} for message in validation_report.warnings[:3]],
        }
    return sorted(
        merged_by_category.values(),
...
def _build_dashboard(
    ...
    if performance_stats["is_load_test"]:
        cards.extend(
            [
                {"key": "total_requests", "label": "Total Requests", "value": performance_stats["total_requests"]},
                {"key": "throughput", "label": "Throughput(rps)", "value": performance_stats["throughput"]},
            ]
        )
    headline = (
        f"{summary['passed_steps']}/{summary['total_steps']} steps passed"
    ...
    return {
        ...
        "summary": {
            ...
            "executed_scenarios": summary["executed_scenarios"],
        ...
        "context_summary": {
            ...
            "defined_key_names": context_stats["defined_keys"],
            "used_keys": context_stats["used_key_count"],
            "used_key_names": context_stats["used_keys"],
            "extracted_keys": context_stats["extracted_key_count"],
            "extracted_key_names": context_stats["extracted_keys"],
            "unused_defined_keys": context_stats["unused_defined_keys"],
            "undefined_used_keys": context_stats["undefined_used_keys"],
        },
...
def _build_task_summary_text(task_name: str, quality_status: str, summary: dict[str, Any]) -> str:
    if summary["execution_status"] == "not_started":
        return (
            ...
            f"已生成 {summary['scenario_count']} 个场景、{summary['total_steps']} 个步骤，"
            "尚未开始执行。"
        )
    return (
        f"任务 {task_name} 当前状态为 {quality_status}，"
...
def _build_task_summary(
    *,
    ...
    if summary["execution_status"] == "not_started":
        ...
        next_action = "优先处理失败步骤与失败原因分类"
    else:
        next_action = "可进入联调或演示环节"
    return {
        "task_name": task_name,
        "quality_status": quality_status,
        "execution_status": summary["execution_status"],
        "headline": task_summary_text,
        "success_rate": summary["success_rate"],
        "failed_steps": summary["failed_steps"],
        "total_requests": summary["total_requests"],
        "throughput": summary["throughput"],
