...
export function TaskDetailArtifactsTab(props: TaskDetailArtifactsTabProps) {
  ...
  if (!artifacts.length) {
    return (
      <div className="task-detail-empty">
        <Text type="secondary">暂无可查看的产物</Text>
      </div>
    );
  }
  ...
  return (
    <List
      ...
      renderItem={(item) => (
        <List.Item>
          <Card className="artifact-card panel-card" bordered={false} title={item.label} onClick={() => void onOpenArtifact(item)}>
            <Text type="secondary">类型：{item.type}</Text>
            <br />
            <Text type="secondary">更新时间：{formatDateTime(item.updated_at)}</Text>
          </Card>
        </List.Item>
      )}
    />
