...
def extract_and_eval(string, extract_list=True):
    opening_char, closing_char = ("[", "]") if extract_list else ("{", "}")
    start = string.find(opening_char)
    if start == -1:
        raise ValueError(
            f"No {'list' if extract_list else 'dict'} can be extracted from: {string}"
        )
    ...
    for index in range(start, len(string)):
        ...
        if char == opening_char:
            ...
            depth -= 1
            if depth == 0:
                literal_string = string[start : index + 1]
                try:
                    return ast.literal_eval(literal_string)
                except (SyntaxError, ValueError) as exc:
                    raise ValueError(
                        "The extracted string is not a valid Python literal: "
                        f"{literal_string}"
                    ) from exc

    raise ValueError(
        f"No complete {'list' if extract_list else 'dict'} can be extracted from: {string}"
    )
