...
const selectors = [
  ...
  '.icon-close',
  '.next-dialog-close',
  '.next-overlay-wrapper [role="button"]',
  '[role="dialog"] [role="button"]',
  '[class*="close"]',
  '[class*="Close"]'
];

const textMatches = ['关闭', '取消', '稍后', '暂不', '知道了', 'close', 'skip', 'x'];
...
for (const el of document.querySelectorAll('button, span, i, div, a')) {
  const text = (el.innerText || el.textContent || '').trim().toLowerCase();
  if (text && textMatches.some((item) => text === item || text.includes(item))) {
    candidates.push(el);
  }
}
...
for (const el of candidates) {
  ...
  try {
    el.click();
    return 'clicked popup close candidate';
  } catch (e) {}
}

return 'no popup close candidate clicked';
"""
        result = self.driver.execute_script(close_popup_script)
        self.driver.wait(self.time_between_actions)
        return close_popup_script + f"\n# Result: {result}"
    ...
    def execute_instruction(self, instruction: str) -> ActionResult:
        ...
        try:
            ...
            elif "CLOSE_POPUP" in instruction:
                code = self._close_popup()
            elif any(
                token in instruction.upper()
                for token in ["ENTER ", "TYPE ", "CLICK ", "SELECT ", "HOVER "]
            ):
                return self.navigation_engine.execute_instruction(instruction)
            else:
